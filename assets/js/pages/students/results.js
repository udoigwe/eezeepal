$(function () {
    ...
    $(document).ready(function($) {
        ...
        $('#form-select-resultsheet').on("submit", function(e){
            ...
            var accessPin = form.find('.access_pin').val();
            ...
            $.ajax({
                url: `${API_URL_ROOT}/result-sheet?semester_id=${semesterID}&user_id=${userID}&access_pin=${accessPin}`,
                type: 'GET',
                dataType: 'json',
    ...
    //load result Sheet
    function loadStudentsGPA(departmentID = '', levelID = '', semesterID = '')
    {
        var table = $('#gpas');
        ...
        table.DataTable({
            ...
            ajax: {
                type: 'GET',
                url: `${API_URL_ROOT}/gpas/data-table/fetch?department_id=${departmentID}&level_id=${levelID}&semester_id=${semesterID}`,
                dataType: 'json',
                headers:{'x-access-token':token},
                complete: function()
                {
            ...
            columns: [
                ...
                },
                {
                    ...
                    {
                        var actions =  `
                            <a href="javascript:void(0);" class="btn btn-link font-18 text-muted btn-sm  btn-print" title="Print Result Details" data-toggle="modal" data-target="#printModal" semester-id="${data}" user-id="${row['user_id']}">
