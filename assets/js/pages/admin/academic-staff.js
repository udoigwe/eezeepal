$(function () {
            ...
            $.ajax({
                ...
                success: function(response)
                {
                    if(response.error == false)
                    {
                        ...
                        if(user.assigned_courses)
                        {
                            var assignedCourses = user.assigned_courses;
                            var assignedCoursesArray = assignedCourses.split(',');

                            for(var i = 0; i < assignedCoursesArray.length; i++)
                            {
                                editModal.find('#courses1').find('option[value="'+assignedCoursesArray[i]+'"]').attr("selected", "selected");
                            }
                        }
    ...
    function loadCourses()
    {
        ...
        $.ajax({
            type:'GET',
            url: userRole == "DAO" ? API_URL_ROOT+'/courses/fetch-by-department/'+departmentID : API_URL_ROOT+'/courses',
            dataType: 'json',
            headers:{ 'x-access-token':token},
            success: function(response)
            {
