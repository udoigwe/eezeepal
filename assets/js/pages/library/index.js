$(function () {
	    ...
	    $.ajax({
	        ...
	        success: function(response)
	        {
	            if(response.error == false)
	            {
	            	...
	            	lendingChart(dashBoard.chart_data);
	            	loadActivities(dashBoard.activities);

	            	//counters
					$('.book-count').text(formatNumber(dashBoard.book_count));
					$('.journal-count').text(formatNumber(dashBoard.journal_count));
					$('.unreturned-count').text(formatNumber(dashBoard.unreturned_count));
	            }
	        },
	...
	function lendingChart(data)
	{
		var total = sum_array(data.datasets.lendingCountArray);
        ...
        var options2 = {
			...
			series: [
				{
					name: 'Lending',
					data: data.datasets.lendingCountArray
				}
		 	],
			labels: data.labels,
			xaxis: {
		...
		var chart2 = new ApexCharts(
			document.querySelector("#lending-report-chart"),
			options2
		);
